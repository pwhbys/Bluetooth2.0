 ...
 * </p>
 * <p>
 * 提供了 {@link Section} 的概念, 用来将列表分块。 具体见 {@link WMGroupListView.Section}
 * </p>
 *
 * @author cginechen
 * @date 2016-10-13
...
public class WMGroupListView extends LinearLayout {


    private SparseArray<Section> mSections;

    public WMGroupListView(Context context) {
        this(context, null);
    }

    public WMGroupListView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public WMGroupListView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public WMCommonListItemView createItemView(@Nullable Drawable imageDrawable, CharSequence titleText, String detailText, int orientation, int accessoryType, int height) {
        WMCommonListItemView itemView = new WMCommonListItemView(getContext());
    ...
    public WMCommonListItemView createItemView(@Nullable Drawable imageDrawable, CharSequence titleText, String detailText, int orientation, int accessoryType) {
        int height;
        if (orientation == WMCommonListItemView.VERTICAL) {
    ...
    public WMCommonListItemView createItemView(CharSequence titleText) {
        return createItemView(null, titleText, null, WMCommonListItemView.HORIZONTAL, WMCommonListItemView.ACCESSORY_TYPE_NONE);
    }

    public WMCommonListItemView createItemView(int orientation) {
        return createItemView(null, null, null, orientation, WMCommonListItemView.ACCESSORY_TYPE_NONE);
    }

    /**
     * private, use {@link Section#addTo(WMGroupListView)}
    ...
    /**
     * private，use {@link Section#removeFrom(WMGroupListView)}
    ...
    /**
     * Section 是组成 {@link WMGroupListView} 的部分。
     * <ul>
     * <li>每个 Section 可以有多个 item, 通过 {@link #addItemView(WMCommonListItemView, OnClickListener)} 添加。</li>
    ...
    public static class Section {
        ...
        private SparseArray<WMCommonListItemView> mItemViews;
        ...
        /**
         * 对 Section 添加一个 {@link WMCommonListItemView}
         *
         * @param itemView        要添加的 ItemView
         * @param onClickListener ItemView 的点击事件
         * @return Section 本身,支持链式调用
         */
        public Section addItemView(WMCommonListItemView itemView, OnClickListener onClickListener) {
         ...
         * @return Section 本身, 支持链式调用
         */
        public Section addItemView(final WMCommonListItemView itemView, OnClickListener onClickListener, OnLongClickListener onLongClickListener) {
        ...
        /**
         * 将 Section 添加到 {@link WMGroupListView} 上
         */
        public void addTo(WMGroupListView groupListView) {
            if (mTitleView == null) {
            ...
            WMCommonListItemView.LayoutParamConfig leftIconLpConfig = new WMCommonListItemView.LayoutParamConfig() {
                @Override
            ...
            for (int i = 0; i < itemViewCount; i++) {
                WMCommonListItemView itemView = mItemViews.get(i);
...
//                        itemView.updateBottomDivider(0, 0, 1, separatorColor);
        ...
        public void removeFrom(WMGroupListView parent) {
