...
/**
 * 作为通用列表 {@link WMGroupListView} 里的 item 使用，也可以单独使用。
 * 支持以下样式:
 * <ul>
 ...
 * @date 2015-1-8
 */
public class WMCommonListItemView extends QMUIConstraintLayout {
    ...
    public WMCommonListItemView(Context context) {
        this(context, null);
    }

    public WMCommonListItemView(Context context, AttributeSet attrs) {
        this(context, attrs, R.attr.QMUICommonListItemViewStyle);
    }

    public WMCommonListItemView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        switch (type) {
            ...
            case ACCESSORY_TYPE_SWITCH: {
                if (mSwitch == null) {
                    ...
                    mSwitch.setThumbResource(com.WideMouth.bluetooth20.R.drawable.switch_thumb_selecter);
                    mSwitch.setTrackResource(com.WideMouth.bluetooth20.R.drawable.switch_track_selecter);
    ...
    public void addAccessoryCustomView(View view, ViewGroup.LayoutParams layoutParams) {
        if (mAccessoryType == ACCESSORY_TYPE_CUSTOM) {
            mAccessoryView.addView(view, layoutParams);
        }
    }
